# Check to see if we are interactive shell
[[ "$-" != *i* ]] && return
##########################################################
...
cGIT1=$LIGHTGREEN # Color for clean git branch
cGIT2=$RED # Color for "" git branch
cGIT3=$YELLOW # Color for "" git branch
...
function promptcmd()
{
        ...
        # GIT information
        #=========================================================
        GITSTS=$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/\1/')
        if [ $GITSTS ] ; then

                clean=`command git status | grep "nothing to commit" | wc -l`
                if [ "$clean" -eq "1" ]; then
                        PS1="${PS1}\342\224\200${cBRACKETS}[${cGIT1}\342\234\227:${GITSTS}${cBRACKETS}]${cLINES}" # Green
                else
                        stagged=`command git status | grep "not staged for commit" | wc -l`
                        if [ "$stagged" -eq "1" ]; then
                                PS1="${PS1}\342\224\200${cBRACKETS}[${cGIT2}\342\234\227:${GITSTS}${cBRACKETS}]${cLINES}" # Red
                        else
                                PS1="${PS1}\342\224\200${cBRACKETS}[${cGIT3}${GITSTS}\342\234\227:${cBRACKETS}]${cLINES}" # Yellow
                        fi
                fi
        fi
